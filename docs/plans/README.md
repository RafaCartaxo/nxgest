...
| [PLAN-080-insights-dashboard.md](PLAN-080-insights-dashboard.md) | **Insights Dashboard (whitelabel `insights` + Recharts)** — novo módulo read-only com gráficos (tendência, performance, fluxo/carteira, ranking), toggle on/off sem consequências, Fase 1 = operador · **depende do PLAN-081** | 📝 Planejado (18/08) |
| [PLAN-081-navegacao-escalavel.md](PLAN-081-navegacao-escalavel.md) | **Navegação escalável (mobile-first)** — tab bar com cap 5 + aba "Mais" (overflow → sheet) + sidebar desktop colapsável (`w-64` ⇄ rail `w-16`), registro único `shared/navigation/nav.ts`; pré-requisito do PLAN-080 | 📝 Planejado (18/08) |
