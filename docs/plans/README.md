...
| [PLAN-080-insights-dashboard.md](PLAN-080-insights-dashboard.md) | **Insights Dashboard (whitelabel `insights` + Recharts)** — módulo read-only com gráficos (tendência, previsto × recebido, fluxo/carteira, contribuição); **Fase 0.5 aplicada (28/08)**: `dependsOn: []` permanente, 4 classes de dado, endpoint único `/api/insights/resumo`, desacoplado do PLAN-081 (entrega sem nav) | ✅ **Implementado F0+F1+F2 (29/08)** · Fase 1.5 (nav) bloqueada no PLAN-081 |
| [PLAN-081-navegacao-escalavel.md](PLAN-081-navegacao-escalavel.md) | **Navegação escalável (mobile-first)** — registro único `shared/navigation/nav.ts` (refactor puro primeiro) + tab bar com cap 5 e aba "Mais" (sheet com title); **revisado 28/08**: Rota permanece primária, `insights` no "Mais", sidebar colapsável **fora de escopo** (BACKLOG) | 📝 Planejado (18/08) |
| [PLAN-082-devboard-recharts.md](PLAN-082-devboard-recharts.md) | **Board DEV com Recharts** — health em um olhar + pass rate/duração trend + falhas por workflow + runs agrupados; agregação no backend (`/api/devboard/resumo`), reusa `ChartCard` do PLAN-080 | 📝 Planejado (18/08) |
| [PLAN-083-otimizacao-consultas-busca.md](PLAN-083-otimizacao-consultas-busca.md) | **Otimização de consultas, round trips e busca** — consolidar agregados (caixa 12→5, dashboard 8→3, cobrança 7→3), eliminar subqueries correlacionadas por linha, bulk inserts, colunas/paginação, índice `lower(email)`, busca sem acento (unaccent) + CPF/telefone/comércio + leads, rawQuery transacional, React Query no front | ✅ **Concluído (19/08)** — Fases 0–9 · smoke 278/278 · em produção |
| [PLAN-084-organizacao-repositorio.md](PLAN-084-organizacao-repositorio.md) | **Organização do repositório** — classificação por responsabilidade, fronteira Git/Docker, sync factual de docs/índices/status, convenção de nomes `nxgest`, referências órfãs e integridade CI/CD | ✅ **Concluído (20/08)** — Blocos 1–5 · `audit:links` + SHA no staging + backup obrigatório |
| [PLAN-085-contrato-periodicidade-alternada.md](PLAN-085-contrato-periodicidade-alternada.md) | **Contratos com periodicidade alternada** — 3º modelo (`alternada`, a cada 2 dias), default 10 parcelas, zero migração, BR-107/BR-108 (estende BR-039 / revoga BR-042-A), espelho backend↔frontend sincronizado | ✅ Implementado (27/08) |
| [PLAN-086-identidade-plataforma.md](PLAN-086-identidade-plataforma.md) | **Identidade de plataforma** — doc canônica (NORTH-STAR/PROJECT/PRD) em 2 níveis + **ADR-007** (critérios de admissão de vertical, tipo_negocio adiado) + 4 strings user-facing (rodapé e-mail, manifest, queroConhecer, `auth.loginSubtitle` nova) | ✅ Implementado (28/08) |
| [PLAN-087-mensagens-falha-convite.md](PLAN-087-mensagens-falha-convite.md) | **Mensagens de falha de convite** — incidente de produção: 6 códigos distintos na ativação (`CONVITE_SUBSTITUIDO` etc.), 6 chaves i18n órfãs ativadas, ação de saída no `AtivarPage`, e-mail com prazo de 7 dias, BR-109 | ✅ Implementado (28/08) |
...
| [Lovable-Perfil-Conta-NXGest.md](Lovable-Perfil-Conta-NXGest.md) | Briefing Lovable: **Perfil & Conta (PLAN-075)** — Meus dados · Operador form/detail/lista · Ativação · Verificação de e-mail · Conta suspensa · Login (v2.0 — telas Stitch portadas 14/08) | ✅ Portado (14/08, Stitch `stitch_personality_plus_portal`) |
