...
- **API/backend:** `scripts/smoke-api.mjs` (**274 cenários** da `07` — validado 18/08) ✅.
- **UI/frontend:** em execução — `PLAN-067-testes.md` (vitest + jsdom + RTL + coverage + CI). **Base atual: 154 testes verdes** (33 arquivos: use-cases auth/leads/conta + shared/segurança + lógica front + UI RTL); CI valida `tsc · build · audits · npm test · docs:audit` + job smoke isolado.
...
- **API:** `scripts/smoke-api.mjs` ✅ (**274 cenários**, validado 18/08)
- **UI (T1):** em execução — `PLAN-067-testes.md` (vitest + jsdom + RTL + coverage + CI; **154 testes verdes** + CI no push)
