...
- [x] **Ajuste de caixa modal**: `tsc` · `build` · audits · `npm test` (78/78) · `docs:audit` · smoke: SC-006b (sócio 201 subárvore) + SC-006c (sócio 404 fora) + CAX-044/045 intactos
...
- [x] **Ajuste de Caixa Total → modal + form compartilhado (11/08)**: `AjusteCaixaForm` (react-hook-form+zod, padrão do projeto) + `AjustarCaixaModal` (CaixaPage botão→modal→salvar) + `AjusteCaixaCard` (OperadorDetail) refatorado p/ o mesmo form. `canAdjust` inclui `socio` (BR-325: subárvore — alinha com o backend que já aceitava). Docs BR-079/02-API/07 atualizadas; smoke SC-006b/c novos (sócio 201 na subárvore · 404 fora) — validados
