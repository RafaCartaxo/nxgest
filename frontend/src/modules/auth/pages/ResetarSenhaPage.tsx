...
import { redefinirSenha } from "../services/auth.service.js"
...
export function ResetarSenhaPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErro(err instanceof ApiError ? err.message : t("auth.erroRedefinir"))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <PublicPageShell>
      <h2 className="font-display text-[22px] font-semibold">{t("auth.resetarTitle")}</h2>
      <p className="mt-1 mb-5 text-sm text-text-secondary">{t("auth.resetarSubtitle")}</p>
      ...
      {tokenInvalido ? (
        <>
          ...
          <div className="mt-4 text-center">
            <Link to="/recuperar-senha" className="text-sm font-medium text-primary hover:underline">
              {t("auth.refazerPedido")}
            </Link>
          </div>
        </>
      ) : feito ? (
        <SuccessState
          title={t("auth.resetarFeito")}
          detail={t("auth.resetarFeitoDetail")}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" disabled={loading} className="w-full">
            {loading ? t("common.loading") : t("auth.salvarSenha")}
