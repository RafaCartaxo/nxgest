...
const { resumoMock } = vi.hoisted(() => ({ resumoMock: vi.fn() }))
...
vi.mock("../hooks/useInsights.js", () => ({ useResumoInsights: () => resumoMock() }))
...
describe("InsightsPage (PLAN-080 F1)", () => {
  ...
  it("série toda zero → empty-state da página (insights.semDados)", () => {
    ...
    render(<InsightsPage />)
    expect(screen.getByText("insights.semDados")).toBeInTheDocument()
  })
})