...
import { db, sqlite, usuarios } from "../../../../database.js"
...
function count(sql: string, ...args: string[]): number {
  const rows = sqlite.prepare(sql).all(...args) as { c: number }[]
...
export class ImpactoDesativacaoQuery implements IImpactoDesativacaoQuery {
  async calcular(empresaId: string, modulosAtuais: ModuleId[], novosModulos: string[]): Promise<ImpactoDesativacao> {
    ...
    if (desligados.includes("clientes")) {
      const n = count(`SELECT COUNT(*) AS c FROM clientes WHERE deletedAt IS NULL AND userId IN (${ph})`, ...args)
      impacto.push({ modulo: "clientes", contagem: n, bloqueia: false, detalhe: "clientes cadastrados" })
    }

    if (desligados.includes("contratos")) {
      const ativos = count(`SELECT COUNT(*) AS c FROM contratos WHERE deletedAt IS NULL AND estado = 'Ativo' AND userId IN (${ph})`, ...args)
      const pendentes = count(
    ...
    if (desligados.includes("caixa")) {
      const n = count(`SELECT COUNT(*) AS c FROM caixa_config WHERE caixaBase != 0 AND userId IN (${ph})`, ...args)
      const aberto = n > 0
    ...
    if (desligados.includes("gastos")) {
      const n = count(`SELECT COUNT(*) AS c FROM gastos WHERE deletedAt IS NULL AND data = ? AND userId IN (${ph})`, hoje, ...args)
      impacto.push({ modulo: "gastos", contagem: n, bloqueia: false, detalhe: "gastos de hoje" })
    }

    if (desligados.includes("rota")) {
      const n = count(`SELECT COUNT(*) AS c FROM historico_operacional h WHERE date(h.createdAt, 'localtime') = ? AND h.userId IN (${ph})`, hoje, ...args)
      impacto.push({ modulo: "rota", contagem: n, bloqueia: false, detalhe: "visita(s) registrada(s) hoje" })
    }

    if (desligados.includes("atendidos")) {
      const n = count(`SELECT COUNT(*) AS c FROM historico_operacional h WHERE h.tipo = 'visitado' AND date(h.createdAt, 'localtime') = ? AND h.userId IN (${ph})`, hoje, ...args)
