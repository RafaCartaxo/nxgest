 ...
 *
 * PLAN-087: cada motivo de falha tem erro/código próprio — a mensagem genérica
 * "Token inválido ou já utilizado" não explica o que fazer (incidente real de produção).
 */
export class AtivarContaUseCase {
  ...
  async execute(input: { token: string; senha: string }): Promise<void> {
    ...
    if (convite.status === "REVOGADO") throw new ConviteRevogadoError()
    if (convite.usadoEm) throw new ConviteJaUsadoError()
    ...
    // se ainda dentro de `expiraEm`, é link substituído → CONVITE_SUBSTITUIDO; se passou
    // do prazo, é vencimento de verdade → TOKEN_EXPIRED.
    if (convite.status === "EXPIRADO") {
      if (vencido) throw new TokenExpiradoError()
      throw new ConviteSubstituidoError()
    }
    ...
    if (usuario.email !== convite.emailAlvo) throw new ConviteEmailNaoConfereError()
